...
mydata_p2$sensor_time <- substr(mydata_p2$sensor_time, 16, 39)
...
df <- cbind(join_parcelles, d1)

df$value <- as.numeric(df$value)
df$date_time <- as.POSIXct(df$date_time)

fg <- plot_ly(x=~df$date_time, y=~df$value, alpha = 0.4,
              mode = 'lines', text = paste("days from today"))
fg
fg1 <- plot_ly(x=~df$date_time, y=~df$value, alpha = 0.4,
              mode = 'lines', text = paste("days from today"))
...
```{r}
library(pracma)
mydata_p2$value <- as.numeric(mydata_p2$value)
pracma::findpeaks(as.numeric(mydata_p2[1:100,]$value))
```

```{r}
subset_df <- mydata_p2[1:1000,]
subset_df$sensor_time <- as.POSIXct(subset_df$sensor_time)

#y <- as.POSIXct(x, format = "%Y-%m-%d %H:%M:%OS3")
#d <- as.POSIXct(sensor_clean_2$time)
subset_df$sensor_time <- format(subset_df$sensor_time,"%Y-%m-%d %H:%M:%OS3")


fg <- plot_ly(x=~subset_df$sensor_time, y=~subset_df$value, alpha = 0.4,
              mode = 'markers', text = paste("days from today"))
fg
```

- largeur pic : vitesse (intervalle pic) // compensation de la perte de donnée due à la déformation 
