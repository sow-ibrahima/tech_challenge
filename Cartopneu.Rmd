...
```{r}
der <-  deriv(join_parcelles$value, join_parcelles$date_time)
```
```{r}
library(xts)
...
```{r}
xxxx <- which(diff(diff(join_parcelles$value, differences=2)))
```
```{r}
library(data.table)
...
```{r}
maxs <- which(maximums(join_parcelles$value))
mins <- which(minimums(join_parcelles$value))
```

```{r}
table(is.na(mydata_p2))
pracma::findpeaks(mydata_p2$value)
```
