...
```{r}
#road_sec$value
roads <- read.csv("P2/roads_p2.csv")
library(quantmod)
part_road_sec=roads #road_sec, seulement l'allé de la seconde section de la route
part_road_sec$value=as.numeric(part_road_sec$value)
part_road_sec$date_time=as.POSIXct(part_road_sec$date_time)
...
minmax$time=part_road_sec$date_time
...
```{r}
library(plotly)
fg <- plot_ly(x=~part_road_sec$time, y=part_road_sec$value, alpha = 0.4,
               text = part_road_sec$id)
fg
