...
library(viridis)
library(leaflet)
domain <- range(field_1$value)
soil_rasters <- raster("classif_sol.tif",prefer_proj = epsg3857)
# make palette

field_1$lat <- as.numeric(field_1$lat)
field_1$long <- as.numeric(field_1$long)
pal <- colorNumeric(palette = viridis(3), domain = domain)
...
```{r}
fields_map <- leaflet(field_1) %>% addProviderTiles(providers$CartoDB.DarkMatterNoLabels,
                      options = providerTileOptions(noWrap = TRUE),  group = "Basic Layer(default)") %>%
                addCircleMarkers(lng = ~long, lat = ~lat,radius = ~(value-330), color = ~pal(value), 
                        fillColor = ~pal(value), opacity = 0.2,
                        label = paste0(field_1$value, " - " ,field_1$soil_type), group = "Values") %>%
